...
function HonestyBadges({ card }: { card: ContractorCard }) {
  return (
    <div className="honesty-badges" aria-label={HONESTY_TEXT.sectionLabel}>
      <span
        className={`honesty-badge ${card.flags.synthetic ? "honesty-badge-synthetic" : "honesty-badge-source"}`}
        title={card.flags.synthetic ? HONESTY_TEXT.syntheticHint : HONESTY_TEXT.sourceHint}
      >
        {card.flags.synthetic ? HONESTY_TEXT.syntheticProfile : HONESTY_TEXT.sourceProfile}
      </span>
      {card.flags.price_imputed && (
        <span className="honesty-badge honesty-badge-estimate" title={HONESTY_TEXT.imputedPriceHint}>
          {HONESTY_TEXT.imputedPrice}
        </span>
      )}
      {card.flags.city_imputed && (
        <span className="honesty-badge honesty-badge-estimate" title={HONESTY_TEXT.imputedCityHint}>
          {HONESTY_TEXT.imputedCity}
        </span>
      )}
    </div>
  );
