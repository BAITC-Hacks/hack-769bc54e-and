...
def _mock_report(results: dict) -> str:
    if not results:
        # Домен не смог разобрать запрос, а инструментов никто не вызывал: без этой ветки
        # проверяющий без ключа увидел бы пустую заглушку и решил, что проект сломан.
        return (
            "## Mock-режим\n\n"
            "Настоящая модель выключена: в `.env` не заданы `OPENAI_API_KEY` и `OPENAI_MODEL`. "
            "Заскриптованный разбор запроса не смог вытащить из него все обязательные параметры.\n\n"
            "Выберите готовый пример над полем ввода — на них mock-режим отрабатывает весь "
            "сценарий целиком, включая вызовы инструментов."
        )
    body = "\n".join(
