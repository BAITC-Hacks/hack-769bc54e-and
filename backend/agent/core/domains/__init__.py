...
Модуль домена обязан объявить:
    ...
    REPORT_FORMAT: str        — необязательно: свой формат итогового ответа вместо общего
    offline_report(results)   — необязательно: ответ без модели, собранный из фактов инструментов
